...
from utils_var import (check_types, check_commensurate, check_intercept,
...
class GLM:
    ...
    *parameter* of the one parameter family of distributions dist.
    ...
    def _fit(self, X, y, *,
        ...
        self.p = X.shape[1]
        self.max_n_iter = n_iter
        self.information_matrix_ = self._compute_ddbeta(X, dmu, var, sample_weights)
        return self



    def Gradient_mask_score(self,X,mask,offset=None):
        ## X: [m,n] covariate for prediction score
        ## mask: [1,n] learnable mask for variable selection
        ## return: gradient [m,1]
        prediction = None
        intercept = np.ones((X.shape[0],1))
        '''
        if offset is None:
            prediction = self.family.inv_link(np.dot(np.concatenate((intercept,X*mask),1), self.coef_))
        else:
            prediction = self.family.inv_link(np.dot(np.concatenate((intercept,X*mask),1), self.coef_) + offset)
        '''
        #nan_flag = np.isnan(prediction).any()
        #print ("FUCKKKKKNAN",np.max(mask))
        #print ((X*mask).shape)
        prediction = np.exp(-np.dot(np.concatenate((intercept,X*mask),1), self.coef_))
        #print (prediction.shape)
        prediction = prediction.reshape(-1,1)
        gradient = prediction*(X*self.coef_[1:])
        #print (gradient.shape)
        return -gradient

    def Gradient_mask_regression(self,X,mask,offset=None):
        
        coffe_covariate = self.coef_[2:]
        #print ("The shape of coffe_covariate is ", coffe_covariate.shape)
        coffe_covariate = coffe_covariate.reshape(1,-1)
        gradient = X*coffe_covariate
        #print (gradient.shape)
